...
router.put("/edit", verifyToken, async(req, res) => {
  if (!req.body.username && !req.body.oldPassword) return res.status(400).send("Invalid fields!")
  
  const userExist = await User.findOne({_id: req.user})
  if (!userExist) return res.status(400).send("No such user!")

  if(req.body.username){
    if(userExist.username.toLowerCase() === req.body.username.toLowerCase()) return res.status(400).send("Type another username!")
    //change username
    userExist.username = req.body.username
  }
  if(req.body.oldPassword && req.body.newPassword){
    //verify old password with current in the db
    const verifyOldPassword = await bcrypt.compare(req.body.oldPassword, userExist.password)
